...
def hamiltonian_coeffs_and_obs(graph):
    ...
    n_matrix = np.array([[1,0],[0,0]])
    for i in range(num_vertices):
        obs.append(qml.Hermitian(n_matrix, wires = i))
        coeffs.append(-1)
...
def train_circuit(num_vertices, H):
    ...
    epochs = 500
    opt = optimize.AdamOptimizer()
    np.random.seed(0)
    params = np.pi * np.random.randn(num_vertices, requires_grad=True)
