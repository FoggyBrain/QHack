...
def error_wire(circuit_output):
    ...
    errorprob = np.zeros(4)
    '''
    if circuit_output[1]>0:
        r = circuit_output[0]/circuit_output[1]
        errorprob[3]=1/(1+r)

    if circuit_output[2]>0:
        r = circuit_output[0]/circuit_output[2]
        errorprob[2]=1/(1+r)

    if circuit_output[7]>0:
        r = circuit_output[0]/circuit_output[7]
        errorprob[1]=1/(1+r)

    errorprob[0]=1-errorprob[1]-errorprob[2]-errorprob[3]
    '''
    errorprob[0] = circuit_output[0]+circuit_output[7]
    errorprob[1] = circuit_output[3]+circuit_output[4]
    errorprob[2] = circuit_output[2]+circuit_output[5]
    errorprob[3] = circuit_output[1]+circuit_output[6]
